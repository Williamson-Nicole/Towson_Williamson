...
## Box plot NDVI 
```{r}
Peak_season_NDVI <- LTER_NDVI_CLEAN %>%
  filter(time=="peak")

Peak_season_NDVI$plot <- factor(Peak_season_NDVI$plot, levels = c("CT", "F2", "F5", "F10"))

ggplot(data=Peak_season_NDVI, aes(x=year, y=avg_ndvi, fill=plot)) +
  scale_fill_manual(values = c("CT" = "#E1BA15", 
                                "F2" = "#41b6c4",
                                "F5" = "#2c7fb8",
                                "F10" = "#253494"))+
...
ggsave("../Figures/LTER_avg_NDVI_bargrpah.jpeg")
