...
TV_2018 <- read.csv("../Data/TeamVole_toolik_2018_calculated_cover.csv", header = TRUE)

TV_2019 <- read.csv("../Data/TeamVole_toolik_2019_calculated_cover.csv", header = TRUE)

TV_2021 <- read.csv("../Data/TeamVole_toolik_2021_calculated_cover.csv", header = TRUE)
...
### Calculate cover my species for 2021

This step sums relative cover across species within each individual quadrat. Just in case changing the naming convection created any duplicates. 
```{r}
TV_2021_calc <- (TV_ALL) %>% filter(year == "2021", species != "bare", species != "std", species != "frost boil", species != "tussock litter", species != "winterkill", species != "litter") %>% group_by(site, treatment, quad, species) %>% summarise_at(vars(cover), list(cover.spec = sum ), na.rm = TRUE)
```
...
TV_2021_avg.quad.spec <- (TV_2021_calc) %>% group_by(site, treatment, species) %>% summarise_at(vars(cover.spec), list(sum.quad = sum), na.rm = TRUE)

#Vector for calculating average
Q8.avg <- c(TV_2021_avg.quad.spec$sum.quad/8)

#creates and fills a new column with the average (by diving that sum by the number of reps specified)
TV_2021_avg.quad.spec["avg.quad"] <-Q8.avg

#export this file
write.csv(TV_2021_avg.quad.spec, file = "../Derived CSVs/TeamVole_2021_relcov_quad_avg.csv")
```
...
TV_2021_avg.spec.site <- (TV_2021_avg.quad.spec) %>% group_by(treatment, species) %>% summarise_at(vars(avg.quad), list(avg.site = mean, site.SD = sd), na.rm = TRUE)

#export this file
write.csv(TV_2021_avg.spec.site, file = "../Derived CSVs/TeamVole_2021_relcov_site_avg.csv")
```
...
ggsave("../Figures/toolik_cover_treatment_avrg.jpeg") #exports a jpeg image of the graph
```
...
ggsave("../Figures/toolik_cover_treatment_site_avrg.jpeg") #exports a jpeg image of the graph
```
