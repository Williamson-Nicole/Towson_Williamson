...
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+
  ...
  labs(y = "Eriophorum", x = "Deciduous shrubs")+ 
...
group_by(year, block, plot) %>%
  summarise(H_quad_avg = mean(H, na.rm = TRUE), n = n(), sd = sd(H, na.rm = TRUE))%>%
  ungroup()
...
H_block_avg <- H_quad_avg %>%
  group_by(year, plot)%>%
  summarise(H_block_avg = mean(H_quad_avg, na.rm = TRUE), n = n(), sd = sd(H_quad_avg, na.rm = TRUE))%>%
  ungroup()
...
group_by(year, block, plot, quad) %>%
  ...
  filter(species =="Bet nan")%>%
    group_by(year, block, plot)%>%
  summarise(rich_avg_quad = mean(n, na.rm = TRUE), n = n(), sd = sd(n, na.rm = TRUE))%>%
  ungroup()

richness_avg_block <- richness_avg_quad %>%
  group_by(year, plot)%>%
  summarise(richness_avg_block = mean(rich_avg_quad, na.rm = TRUE), n = n(), sd = sd(rich_avg_quad, na.rm = TRUE))%>%
  ungroup()
