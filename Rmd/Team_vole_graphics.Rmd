...
`r colorize("*All graphics were created by **Nicole Williamson**. Please notify creator before using, and cite credit for the work accordingly.*", "red")`
...
```{r, echo=FALSE}
xfun::embed_file('../Images/Tundra_model.png', text = 'Click HERE to download image')
```
...
```{r, echo=FALSE}
xfun::embed_file('../Images/Team_vole_experimental_design.png', text = 'Click HERE to download image')
```  
...
```{r, echo=FALSE}
xfun::embed_file('../Images/Vole_activity.png', text = 'Click HERE to download image')
```  
